...
class WhatsApp:
    ...
    def nav_input_box(self):
        try:
            position = pt.locateOnScreen('smily_paper.png', confidence=.7)
            pt.moveTo(position[0:2], duration=self.speed)
    ...
    def nav_message(self):
        try:
            position = pt.locateOnScreen('smily_paper.png', confidence=.7)
            pt.moveTo(position[0:2], duration=self.speed)
            pt.moveRel(80, -30, duration=self.speed)  # x,y has to be adjusted depending on your computer
        except Exception as e:
    ...
    def get_message(self):
        ...
        sleep(self.speed)
        pt.moveRel(12, 20, duration=self.speed)  # x,y has to be adjusted depending on your computer
